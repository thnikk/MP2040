...
// Replacements for the GP2040-CE PeripheralI2C calls: the pico-sdk hardware
// i2c functions take the same address/buffer/length/nostop arguments.
static int16_t i2c_write_wrap(i2c_inst_t* i2c, uint8_t addr, const uint8_t* data, uint16_t len, bool nostop) {
	return i2c_write_blocking(i2c, addr, data, len, nostop);
}

static int16_t i2c_read_wrap(i2c_inst_t* i2c, uint8_t addr, uint8_t* data, uint16_t len, bool nostop) {
	return i2c_read_blocking(i2c, addr, data, len, nostop);
}

static int16_t i2c_read_register_wrap(i2c_inst_t* i2c, uint8_t addr, uint8_t reg, uint8_t* data, uint16_t len) {
	uint8_t cmd = reg;
	if (i2c_write_blocking(i2c, addr, &cmd, 1, true) != 1) return -1;
...
void GPGFX_TinySSD1306::init(GPGFX_DisplayTypeOptions options) {
	...
	// Read the display RAM address counter low nibble: 0x08 = 132 columns
	// (SH1106), 0x03/0x06/0x07 = 128 columns (SSD1306 variants).
	this->screenType = 0;
	i2c_read_register_wrap(_options.i2c, _options.address, 0x00, &this->screenType, 1);
...
// SH1106 detection: write/read back five known bytes via the read-modify-write
// I2C sequence. If the panel echoes what we wrote, it's a RAM-accessible
// SH1106 (132x64).
bool GPGFX_TinySSD1306::isSH1106() {
	this->setPower(false);
	...
	for (; i < sizeof(RANDOM_DATA); ++i) {
		...
		if (i2c_write_wrap(_options.i2c, _options.address, buffer, 3, false) == 0) {
			break;
		}

		// Read two bytes back, the first byte is a dummy read and the second byte is the byte was actually want.
		if (i2c_read_wrap(_options.i2c, _options.address, buffer, 2, false) == 0) {
			break;
		}
		...
		if (i2c_write_wrap(_options.i2c, _options.address, buffer, 4, false) == 0) {
			break;
		}
	}
...
void GPGFX_TinySSD1306::drawText(uint8_t x, uint8_t y, std::string text, uint8_t invert) {
	...
	for (uint8_t charIndex = 0; charIndex < text.size() && charIndex < maxTextSize; charIndex++) {
