 ...
 *
 * Adapted for MP2040: no USB host / auth passthrough, so all console
 * authentication logic is stripped; the GIP announce -> descriptor handshake is
 * kept because the Windows Xbox One driver (xboxgip, bound via the XGIP10 MS OS
 * descriptor) requires it. Gamepad state comes from Storage.keyState via the
 * shared gamepad helper; A1 = guide (GIP virtual keycode), A2 = share/capture.
 */
...
bool xbone_vendor_control_xfer_cb(uint8_t rhport, uint8_t stage,
	...
	// Only claim the MSFT OS descriptor request. Everything else is left to
	// the other class drivers (the built-in CDC driver must see its own
	// class-specific requests, e.g. SET_LINE_CODING).
...
bool xbone_xfer_cb(uint8_t rhport, uint8_t ep_addr, xfer_result_t result,
    ...
    // Do nothing if we couldn't setup our XGIPs
    if ( incomingXGIP == nullptr || outgoingXGIP == nullptr) {
        return true;
    }
    ...
    if (ep_addr == p_xbone->ep_out) {
        ...
        if ( command == GIP_ACK_RESPONSE ) {
            ...
            // Power Mode On! (nothing to surface without host auth)
...
void XBoneDriver::process() {
    ...
    // No input until the announce -> descriptor handshake completes
    if ( xboneDriverState != AUTH_DONE ) {
    ...
    // Assemble the gamepad state (dpad + buttons + sticks)
    ...
    const bool guide = (gamepad.buttons & GAMEPAD_MASK_A1) != 0;
    if ( (xb1_guide_pressed == true && !guide) ||
        (xb1_guide_pressed == false && guide) ) {
    ...
    newInputReport.a = (gamepad.buttons & GAMEPAD_MASK_B1) != 0;
    newInputReport.b = (gamepad.buttons & GAMEPAD_MASK_B2) != 0;
    newInputReport.x = (gamepad.buttons & GAMEPAD_MASK_B3) != 0;
    newInputReport.y = (gamepad.buttons & GAMEPAD_MASK_B4) != 0;
    newInputReport.leftShoulder = (gamepad.buttons & GAMEPAD_MASK_L1) != 0;
    newInputReport.rightShoulder = (gamepad.buttons & GAMEPAD_MASK_R1) != 0;
    newInputReport.leftThumbClick = (gamepad.buttons & GAMEPAD_MASK_L3) != 0;
    newInputReport.rightThumbClick = (gamepad.buttons & GAMEPAD_MASK_R3) != 0;
    newInputReport.start = (gamepad.buttons & GAMEPAD_MASK_S2) != 0;
    newInputReport.back = (gamepad.buttons & GAMEPAD_MASK_S1) != 0;
    newInputReport.guide = 0; // always 0
    newInputReport.sync = 0;
    newInputReport.share = (gamepad.buttons & GAMEPAD_MASK_A2) != 0;
    newInputReport.dpadUp = (gamepad.dpad & GAMEPAD_MASK_UP) != 0;
    newInputReport.dpadDown = (gamepad.dpad & GAMEPAD_MASK_DOWN) != 0;
    newInputReport.dpadLeft = (gamepad.dpad & GAMEPAD_MASK_LEFT) != 0;
    newInputReport.dpadRight = (gamepad.dpad & GAMEPAD_MASK_RIGHT) != 0;

    // Digital triggers: full on while held, off otherwise.
    newInputReport.leftTrigger = (gamepad.buttons & GAMEPAD_MASK_L2) ? 0x03FF : 0;
    newInputReport.rightTrigger = (gamepad.buttons & GAMEPAD_MASK_R2) ? 0x03FF : 0;

    // Sticks are signed, centered around 0, so offset the 16-bit centered
    // values by INT16_MIN (same convention as the XInput driver).
    newInputReport.leftStickX = (int16_t)gamepad.lx + INT16_MIN;
    newInputReport.leftStickY = (int16_t)gamepad.ly + INT16_MIN;
    newInputReport.rightStickX = (int16_t)gamepad.rx + INT16_MIN;
    newInputReport.rightStickY = (int16_t)gamepad.ry + INT16_MIN;
...
const uint8_t * XBoneDriver::get_descriptor_device_cb() {
    return Storage::getInstance().getSerialConfigEnabled()
        ? xbone_serial_device_descriptor
...
const uint8_t * XBoneDriver::get_descriptor_configuration_cb(uint8_t index) {
    return Storage::getInstance().getSerialConfigEnabled()
        ? xbone_serial_configuration_descriptor
