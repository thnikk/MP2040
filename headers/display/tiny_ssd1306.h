...
// 1-bpp SSD1306 / SH1106 driver. Owns its own 1024-byte framebuffer (kept
// static, not on the 4KB core-1 stack) and flushes it over I2C. SH1106
// (132x64) is auto-detected at init via a read-modify-write probe; it gets a
// 2-pixel x-offset so drawing code still addresses 128x64.
...
public:
	...
	bool isSPI() { return false; }
	bool isI2C() { return true; }
...
private:
	...
	// Framebuffer is a member, not a stack local. The DisplayController holds
	// the driver instance as a member (BSS), so this 1KB never lands on the
	// 4KB core-1 stack.
