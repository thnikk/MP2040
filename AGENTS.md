...
- `-c` clean build, `-s` skip build (reuse existing UF2 for the board), `-v` verbose, `-f` flash to board, `-n` nuke first, `-p <path>` flash mount, `-w` flash a UF2 that boots once into web config mode
...
- Kill your own test servers by **port, not process name**: `fuser -k <port>/tcp` (then verify with `ss -ltnp`). **Never `pkill -f "vite --port <port>"`** — the pattern matches the shell running your own command line, which kills the shell mid-command (the tool call hangs and everything you started dies with it). A fresh mock server is `setsid nohup npm run dev -- --port <port> > /tmp/opencode/<name>.log 2>&1 < /dev/null & disown` so it survives the tool call. Note the mock persists config in memory across requests, so restart it (via the port) between test runs that expect a clean slate.
